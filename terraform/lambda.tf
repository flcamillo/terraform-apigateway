...
# associa a policy da aws para permissão de execução na vpc
resource "aws_iam_role_policy_attachment" "attach_lambda_role_vpc_policy" {
  role       = aws_iam_role.lambda_role.name
...
resource "aws_lambda_function" "lambda" {
  ...
  vpc_config {
    subnet_ids         = var.lambda_vpc_subnet
    security_group_ids = var.lambda_vpc_sg
  }  
  environment {
    variables = {
      DB_CONNECTION_STRING = "mongodb://${var.db_user}:${random_password.dbpassword.result}@${aws_docdb_cluster.cluster.endpoint}:${aws_docdb_cluster.cluster.port}/?ssl=true"
      DB_NAME              = var.db_name
    }
  }
}
